...
`;
  ...
  it("prefers .env over docker-compose.yml", () => {
    ...
    fs.writeFileSync(path.join(tmpDir, "docker-compose.yml"), "VIRTUAL_HOST=from-compose.local\n");
    const result = detectProjectUrl(tmpDir);
  ...
  it("reads VIRTUAL_HOST from docker-compose.yml", () => {
    fs.writeFileSync(path.join(tmpDir, "docker-compose.yml"), "VIRTUAL_HOST=mysite.local\n");
    const result = detectProjectUrl(tmpDir);
    expect(result).toEqual({
