...
import { isHttpServerOwner, getSessionClients } from "./session-store.ts";
...
**Tip:** You can also use the browser extension to toggle the widget without modifying files. Run \`setup_extension\` for instructions.
      ...
      case "get_connection_status": {
        ...
        if (!isHttpServerOwner()) {
          const status = (await proxy.fetchServerStatus(sessionId)) as {
      ...
      case "request_annotation": {
        const message =
          (args.message as string) || "Please annotate the issue you'd like to report.";
